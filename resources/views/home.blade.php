...
<script>
    $(document).ready(function() {
        ...
        $.ajax({
            ...
            success: function(response) {
                ...
                Highcharts.chart('cancers_report', {
                    chart: { type: 'column' },
                    title: true, xAxis: { categories: names },
                    colors: ['#5F73F2'], yAxis: { min: 0, title: true }, 
                    plotOptions: { column: { stacking: 'column' } }, 
                    series: [ { name: '', data:numbers }]
                });
            },
        }); 
    });
    $(document).ready(function() {
        ...
        names=[];
        $.ajax({
            url: "{{route('home')}}?chart=protocols",
            type: 'get',
            data: null,
            processData: false,
            contentType: false,
            success: function(response) {
                var ctx = document.getElementById("protocol_report").getContext('2d');
                names  = response.data.names;
                numbers = response.data.numbers;
